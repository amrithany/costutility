...
def extract_idn_data(request):
    ...
    i = 1
    #for mapptable in IdentifyTable.objects.all():
    while i < 2:
        Temp_Mapping.objects.all().delete()
        Cri_Temp_Mapping.objects.all().delete()
        mapptable = IdentifyTable.objects.filter(dec_id=767).last()  
        #dec_id = 761 
        ...
        '''while len(c):
           ...
           for temp in Temp_Mapping.objects.all():
             if scr_id <>  0 and scr_id is not None and scr_id <> ' ' and scr_id <> '':  
                try:
                   m = Identify_Data.objects.get(dec_id = dec_id, sol_id = temp.sol_id, ec_id = scr_id)
                except ObjectDoesNotExist:
                   m = Identify_Data(dec_id = dec_id, sol_id = temp.sol_id, sol_position = temp.sol_position, sol_option = temp.sol_option, ec_id = cri_id, criterion = criteria, created_date = mapptable.created_date,created_by = mapptable.created_by,updated_date = mapptable.updated_date,updated_by = mapptable.updated_by) 
                   m.save()
