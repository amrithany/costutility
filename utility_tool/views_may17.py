...
from .models import Decisions, Solution_Options, Screening_Criteria, Evaluation_Criteria, Importance_Scores, Users, Stakeholders, Stakeholders_Decisions, MappingTable, SummaryTable, Evaluation_Measures, PA_Setup, EvaluationTable,  Cost_Setup, Cost_Utility, Decision_Made, Detailed_Costs, CBCSE_Screening_Criteria, Master_Screening_Criteria, CBCSE_Evaluation_Criteria, Master_Evaluation_Criteria, Scores_Setup, IdentifyTable, Solution_Options_Storage, Login, SD_dec_file, SD_dec_link, SD_st_file, SD_st_link, SD_solopt_file, SD_solopt_link, SD_scr_file, SD_scr_link, SD_mapp_file, SD_mapp_link, SD_eva_file, SD_eva_link, SD_iw_file, SD_iw_link, SD_evam_file, SD_evam_link, SD_cost_file, SD_cost_link,SD_makedec_file, SD_makedec_link, Duplicated_DecIds, SharedDec, Mapping_Data, Temp_Mapping, Scr_Temp_Mapping  
import datetime
import json
import xlrd
import xlwt
...
def export_all(request):     
    ...
    sql2 = """SELECT dec_id, sol_id, sol_option, sc_id, criterion, result, created_date, created_by, updated_date, updated_by FROM utility_tool_mapping_data where dec_id not in %(demo_dec_list)s and sol_option not like %(slist)s"""                
    ...
    try:
       cursor2.execute(sql2,{'demo_dec_list' :demo_dec_list, 'slist': '%has been deleted'})
       results = cursor2.fetchall()
       if results != ():
          for col_num in xrange(len(columns)):
             ws.write(row_num, col_num, columns[col_num][0], font_style)
...
def extract_data(request):
    ...
    for mapptable in MappingTable.objects.all():
        ...
        Scr_Temp_Mapping.objects.all().delete() 
        ...
        while len(c):
           ...
           for ly in arr3.split(','):
             ...
             if pos2 > 1 and pos2 < max_pos:
               ...
               if scr_id <>  0 and scr_id is not None and scr_id <> ' ' and scr_id <> '':
                  stm = Scr_Temp_Mapping(sc_id = scr_id, position = pos2, value = l4)
                  stm.save()
             pos2 = pos2 + 1
           ...
           for temp in Scr_Temp_Mapping.objects.all():    
              try:  
                 m = Mapping_Data.objects.get(sol_position = temp.position, sc_id = temp.sc_id, dec_id = dec_id)                                                          
                 m.result = temp.value
