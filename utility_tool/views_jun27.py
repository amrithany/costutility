...
def tutorials_dm(request):                                                                                                                               
    ...
    return render(request, 'tutorials_dm.html')      
...
def usageinfo(request):     
    ...
    iw = Stakeholders_Decisions.objects.filter(iw_type = 'Y',votes__gt=10).values_list('dec_id',flat = True).distinct()
    iw_count =iw.count()
    print 'iw_count'
    print iw_count
    ave_iw = round(float(iw_count) / float(dec_count),2) * 100   
    iw_real = Stakeholders_Decisions.objects.filter(iw_type = 'Y', dec_id__in=real_dec_list, votes__gt=10).values_list('dec_id',flat = True).distinct()    
...
def summary_report(request):
    ...
    if total_solopt_count > 0: 
       ...
       if total_solopt_count > 1:
          d.add_run(p4.number_to_words(total_solopt_count).capitalize() + ' possible Solution Options were initially considered but only ' + p4.number_to_words(solopt_count) + ' made it through the Screening process.')
       else:    
...
def appendices(request):     
    ...
    col_num = 0
    #Heading
    ws.write(0, 0, "Appendix A: Stakeholders Participating in this Decision", font_style5)
    counter = 1
    while counter < 8:
       ...
       for row in results:
          ...
          for col_num in xrange(len(row)):
              if col_num == 0 or col_num == 1 or col_num == 2 or col_num == 3:
                 ws.write(row_num, col_num, StValue(row[0], row[1], col_num),font_style6)
              else:
    ...
    if table <> 'doesnotexist':
       ...
       ws.write(1, 0, "The table below indicates whether each Solution Option met or “passed” each Screening Criterion:", font_style6)
       counter = 1
       while counter < s1_count:
          ws.write(1, counter, "", font_style6)
          counter = counter + 1
    ...
    if one_stakeholder == 'Y':
       ws.write(1, 0, "The table below shows a summary of Importance Scores for Evaluation Criteria", font_style6)
        ...
        if vote_changed == 'Y': 
           ws.write(1, 0, "The table below shows a summary of the Importance Scores for Evaluation Criteria provided by each stakeholder Stakeholders were given a different number of votes in determining the relative importance of each Evaluation Criterion. The more votes a stakeholder is assigned, the more this stakeholder influences the final importance weights", font_style6)        
        else:   
           ws.write(1, 0, "The table below shows a summary of the Importance Scores for Evaluation Criteria provided by each stakeholder. Stakeholders were each given an equal number of votes in determining the overall importance weights", font_style6)   
    counter = 1
    while counter < eva_count:
        ws.write(1, counter, "", font_style6)
        counter = counter + 1
    ...
    for st in stdec:
       ...
       counter = counter + 1
       percvotes = (st.votes / total_votes) * 100
...
def export_all(request):     
    ...
    database = MySQLdb.connect (host="amritha.mysql.pythonanywhere-services.com", user = "amritha", passwd = "lilies19", charset="utf8", db = "amritha$costutility")
    cursor = database.cursor ()
    sql = """SELECT d.id, short_title, title,  name_decisionmaker, type_of_dec,  decision_prob,  evidence,  goal,  target_audience,  stakeholders, participating_stakeholders,  potential_sources,  by_when,  real_dec_yn,  created_date,  created_by,  updated_date,  updated_by,  shared, state, zip, country, organisation, position, type_of_org, other_org, other_pos, education, age, gender, race, other_race, publicOrPrivate FROM utility_tool_decisions d, utility_tool_users u where d.created_by = u.user"""
    ...
    row_num = 1 
    cursor2 = database.cursor ()
    sql2 = """SELECT id,  st_id,  dec_id,  iw_type,  iw_date,  evacr_type,  evacr_date,  scrcr_type,  scrcr_date,  solopt_type,  solopt_date,  created_date,  created_by,  updated_date,  updated_by,  votes,  deleted FROM utility_tool_stakeholders_decisions"""      
    ...
    row_num = 1 
    cursor2 = database.cursor ()
    sql2 = """SELECT id, sol_option, source, option_details, filename1, file_attachment1, filename2, file_attachment2, filename3, file_attachment3, filename4, file_attachment4, linkname1, link1, linkname2, link2, linkname3, link3, linkname4, link4, dec_id, archived, archived_date, archived_by, unarchived, unarchived_date, unarchived_by, deleted, created_date, created_by, updated_date, updated_by FROM utility_tool_solution_options"""      
    ...
    row_num = 1 
    cursor2 = database.cursor ()
    sql2 = """SELECT id, criterion,criterion2,dec_id,fieldname,created_date,created_by, updated_date, updated_by FROM utility_tool_screening_criteria"""                    
    columns = [
          (u"Id", 3000),
          (u"Criterion", 8000),
          (u"Criterion2", 8000),
    ...
    row_num = 1 
    cursor2 = database.cursor ()
    sql2 = """SELECT * FROM utility_tool_mappingtable"""
    ...
    row_num = 1 
    cursor2 = database.cursor ()
    sql2 = """SELECT * FROM utility_tool_evaluation_criteria"""
    ...
    row_num = 1 
    cursor2 = database.cursor ()
    sql2 = """SELECT * FROM utility_tool_importance_scores"""
