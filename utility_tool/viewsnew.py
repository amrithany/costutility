...
def pa_setup(request):
    ...
    return render(request,'decisions/pa_setup.html',{'dec_id':dec_id,'dec_title':dec_title, 'stakeholdersNow':stakeholdersNow, 'votesform':votesform,'total_votes':total_votes, 'total_voters':qset_count})
...
def add_solopt_det(request):
           ...
           for recs in id:
              if recs.sol_option <> '':
                 ...
                 try:
                    ...
                    if st2_count > 0:
                       request.session['error_message'] = 'The Option "' + recs.sol_option + '" already exists for this Decision and it has been removed. Please enter an unique Option.' 
    ...
    if 'error_message' in request.session: 
       return render(request,'decisions/solution_options/add_solopt_det.html',{'soloptform':soloptform,'dec_id':dec_id, 'dec_title':dec_title, 'mapping_allowed': mapping_allowed, 'scr_count':scr_count,'created_by':created_by, 'loggedinuser':loggedinuser, 'err':request.session['error_message']})
    else:
           ...
           if old_eva_exists == 'N': 
              ...
              eva_save.save() 
               ...
               if old_eva_exists == 'D':
                  ...
                  old_eva.save(update_fields=['criterion2','orig_eva_id','deleted','updated_by','updated_date'])
               else:   
                  old_eva.criterion2 = request.POST.get(fieldname)
                  old_eva.orig_eva_id = value
                  old_eva.updated_by = request.session['user']
               ...
               try:
                  isw = Importance_Scores.objects.get(eva_id = old_eva.id, dec_id=dec_id, created_by = loggedinuser)
                  isw.criterion = old_eva.criterion + ' ' + old_eva.criterion2
                  if old_eva_exists == 'D':  
                     isw.deleted = 'N'
                  else:
               ...
               try: 
                  for evm in Evaluation_Measures.objects.filter(eva_id = old_eva.id, dec_id=dec_id):
                      evm.criterion = old_eva.criterion + ' ' + old_eva.criterion2
                      if old_eva_exists == 'D':  
                         evm.deleted = 'N'
                      else:
...
def utility_results(request):
    ...
    return render(request,'decisions/solution_options/utility_results.html', {'dec_id':dec_id, 'dec_title': dec_title, 'table':table, 'evam':qset1, 'util_res':util_res, 'solopt':solopt, 'eva_count':eva_count, 'solopt_count':solopt_count, 'imp_scores':imp_scores, 'eva':eva})
