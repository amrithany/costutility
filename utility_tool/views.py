...
def summary_report(request, dec_id):
    ...
    if scrcr_count > 0: 
       ...
       for s in scrcr:
           ...
           document.add_paragraph(criteria, style='List Bullet')
    else:
        run = d.add_run()  
        run.add_break()
        run.add_break()      
...
def dupl(request, dec_id, dtype):
    ...
    #else:    
    strpart = str(dec.created_date)  
    datepart = strpart[0:19]
...
def change_password(request):
    ...
    if request.method == 'POST':
       ...
       if changeform.is_valid():
          ...
          if change.oldpassword != r.password:
             ...
             return render(request,'users/change_password.html',{'changeform':changeform, 'loggedinuser':loggedinuser, 'err': 'Password and Confirmed Password do not match.'})      
          r = Users.objects.get(user = loggedinuser)                                                                                  
          r.password = change.password
          r.passwordagain = change.passwordagain
          r.updated_by = loggedinuser
    ...
    return render(request,'users/change_password.html', {'changeform':changeform, 'loggedinuser':loggedinuser})
...
def renew_license(request):
   ...
   if request.method == 'POST':
      ...
      if licenseform.is_valid():
         license = licenseform.save(commit=False)
         if license.licenseSigned == 'Yes':
            users = Users.objects.get(user = request.session['user'])
            users.licenseSigned = 'Yes'
            users.orig_reg_date = users.startDate
            users.startDate = datetime.datetime.now()
            users.endDate= datetime.datetime.now() + relativedelta(years=2)  
            users.save(update_fields=['orig_reg_date','startDate', 'endDate', 'licenseSigned'])     
            users.save()
            #return HttpResponseRedirect('/utility_tool/users/login.html')
            return HttpResponseRedirect('/utility_tool/decisions/decisions_list.html')   
         else:
      ...
      licenseform = License()                                                                                                             
   return render(request, 'users/renew_license.html',{'licenseform': licenseform, 'publicOrPrivate':'publicOrPrivate'})
...
def export_users(request):
    ...
    columns = [
          ...
          (u"Original Registration Date", 7000),
    ]
...
def export_test(request):                                                                                                                 
    ...
    for m in mapp: 
        ...
        #t = temp_table(field1 = m.id) 
        #t.save()   
        ws.write(row_num, col_num, m.id,font_style4)    
        row_num = row_num + 1
    # disconnect from server
    database.close()                                                                                                                      
...
def export_all(request):     
    ...
    #sql = dec.exclude(id__in=demo_dec_list) 
    #Heading of tables
    a = xlwt.Alignment()
    a.wrap = True
    a.vert = a.VERT_TOP
    ...
    columns = [
          (u"decision_id", 3000),
          (u"decision_type", 3500),
          (u"downloaded_demo_decision", 0),    
          (u"short_title", 3000),
          (u"problem_description", 7000),
          (u"decision_maker_name", 7000),
          (u"decision_problem", 9000),
          (u"evidence", 9000),
          (u"goal", 9000),
          (u"audience_served", 9000),
          (u"stakeholders", 6000),
          (u"participating_stakeholders", 9000),
          (u"total_stakeholders_inc_pa", 9000),
          (u"so_sources", 7000),
          (u"decision_made_by", 5000),
          (u"decision_created_date", 5500),
          (u"decision_created_by", 5000),
          (u"decision_updated_date", 5500),
          (u"decision_updated_by", 5000),
          (u"shared", 3000),
          (u"public_link_sos", 9000),
          (u"total_stakeholders", 9000),
          (u"options_chosen", 4000), 
          (u"explain_decision", 9000), 
          (u"primary_factors", 9000), 
          (u"other_considerations", 9000), 
          (u"cost_metric", 3000),
          (u"cost_source", 9000),
          (u"organization", 3000), 
          (u"position", 3000), 
          (u"org_type", 3000), 
          (u"other_org", 3000), 
          (u"other_position", 4000), 
          (u"education", 3000), 
          (u"age", 3000), 
          (u"gender", 3000), 
          (u"race", 3000), 
          (u"other_race", 3000), 
          (u"institution_type", 4000), 
    ]
    ...
    try:
       ...
       for row in results:
          ...
          short_title = row[3]
          title = row[4]
          name_decisionmaker = row[5]
          decision_prob = row[6]
          evidence = row[7]
          goal = row[8]
          target_audience = row[9]
          stakeholders = row[10]
          participating_stakeholders = row[11]
          potential_sources = row[12]
          by_when = row[13]
          created_date = row[14]
          created_by = row[15]
          updated_date = row[16]
          ...
          for col_num in xrange(len(row)):
              if col_num == 12:                                                                                        
                ...
                ws.write(row_num, col_num, LocVal(row[0]),font_style4)
              elif col_num == 21:                                                                                        
                ws.write(row_num, col_num, TotalST(row[0],'N'),font_style4)  
              elif col_num == 22:
                if row[col_num] is not None:    
                   ws.write(row_num, col_num, CleanupOpt(row[col_num]),font_style4)  
                else:
                   ws.write(row_num, col_num, row[col_num],font_style4)    
              else:
                ws.write(row_num, col_num, row[col_num],font_style4)
    ...
    columns = [
          (u"decision_id", 3000),
          (u"uniquesh_forthisdecision_id", 6500),
          (u"stakeholder_id", 5000),
          (u"pa", 1000),  
          (u"imp_scr_allowed", 5000),
          (u"imp_scr_email", 5000),
          (u"imp_scr_participated", 5000),
          (u"imp_scr_textbox", 10000),
          (u"eval_crit_allowed", 5000),
          (u"eval_crit_email", 5000),
          (u"eval_crit_participated", 6000),
          (u"screening_crit_allowed", 5500),
          (u"screening_crit_email", 5000),
          (u"screening_crit_participated", 5000), 
          (u"sol_opt_allowed", 5000),
          (u"sol_opt_email sent", 5000),
          (u"sol_opt_participated", 5000), 
          (u"stakeholder_created_on", 6000),
          (u"stakeholder_created_by", 6000),
          (u"stakeholder_updated_on", 6000),
          (u"stakeholder_updated_by", 6000),
          (u"number_votes", 4000),
          (u"stakeholder_deleted", 5000),
          (u"sh_organization", 5000),
          (u"sh_position", 3000),
          (u"sh_org_type", 3000),
          (u"sh_other_org", 3500),
          (u"sh_other_position", 5000),
          (u"sh_education", 3500),
          (u"sh_age", 3000),
          (u"sh_gender", 3000),
          (u"sh_race", 3000),
          (u"sh_other_race", 5000),
          (u"sh_institution_type", 5000),
    ]
    ...
    for s in sd:
        row_num += 1
        ws.write(row_num, 0, s.dec_id,font_style4)
        ws.write(row_num, 1, s.id,font_style4)
        ws.write(row_num, 2, s.st_id,font_style4)
        ws.write(row_num, 3, s.PA,font_style4)   
        ws.write(row_num, 4, s.iw_type,font_style4)
        ws.write(row_num, 5, s.iw_date,font_style4)
        ws.write(row_num, 6, Participate(s.dec_id,s.id,6),font_style4)
        #iwcount = Importance_Scores.objects.filter(dec_id = s.dec_id, created_by =s.created_by).count()
        #if iwcount > 0:
           #ws.write(row_num, 6, 'Y',font_style4)
        for ss in Scores_Setup.objects.filter(dec_id = s.dec_id):
            ws.write(row_num, 7, ss.thinking,font_style4)  
        ws.write(row_num, 8, s.evacr_type,font_style4) 
        ws.write(row_num, 9, s.evacr_date,font_style4)
        ws.write(row_num, 10, Participate(s.dec_id,s.id,10),font_style4)
        #evacount = Evaluation_Criteria.objects.filter(dec_id = s.dec_id, created_by =s.created_by).count()
        #if evacount > 0:
           #ws.write(row_num, 10, 'Y',font_style4)   
        ws.write(row_num, 11, s.scrcr_type,font_style4)
        ws.write(row_num, 12, s.scrcr_date,font_style4)   
        ws.write(row_num, 13, Participate(s.dec_id,s.id,13),font_style4)
        #scrcount = Screening_Criteria.objects.filter(dec_id = s.dec_id, created_by =s.created_by).count()
        #if scrcount > 0:
           #ws.write(row_num, 13, 'Y',font_style4)   
        ws.write(row_num, 14, s.solopt_type,font_style4)
        ws.write(row_num, 15, s.solopt_date,font_style4)
        ws.write(row_num, 16, Participate(s.dec_id,s.id,16),font_style4)
        #solcount = Solution_Options.objects.filter(dec_id = s.dec_id, created_by =s.created_by).count()
        #if solcount > 0:
           #ws.write(row_num, 16, 'Y',font_style4)   
        cdate = str(s.created_date)
        sliced_cdate = cdate[0:10]
        ws.write(row_num, 17, sliced_cdate,date_style4)
        ws.write(row_num, 18, IdforUser(s.created_by),font_style4)
        udate = str(s.updated_date)
        sliced_udate = udate[0:10]
        ws.write(row_num, 19, sliced_udate,date_style4)
        ws.write(row_num, 20, IdforUser(s.updated_by),font_style4)
        ws.write(row_num, 21, s.votes,font_style4)
        ws.write(row_num, 22, s.deleted,font_style4)
        u = Users.objects.get(user = s.created_by)
        ws.write(row_num, 23, u.organisation,font_style4)
        ws.write(row_num, 24, u.position,font_style4)
        ws.write(row_num, 25, u.type_of_org,font_style4)
        ws.write(row_num, 26, u.other_org,font_style4)
        ws.write(row_num, 27, u.other_pos,font_style4)
        ws.write(row_num, 28, u.education,font_style4) 
        ws.write(row_num, 29, u.age,font_style4)
        ws.write(row_num, 30, u.gender,font_style4)
        ws.write(row_num, 31, u.race,font_style4) 
        ws.write(row_num, 32, u.other_race,font_style4)
        ws.write(row_num, 33, u.publicOrPrivate,font_style4)
        #s.dec_id, s.id,  st_id,  pa, iw_type,  iw_date, '', thinking, evacr_type,  evacr_date, '', scrcr_type,  scrcr
        #_date, '', solopt_type,  solopt_date,  '', s.created_date,  s.created_by,  s.updated_date,  s.updated_by,  votes,  deleted, organisation, position, type_of_org, other_org, other_pos, education, age, gender, race, other_race, publicOrPrivate
    '''try:
       ...
       for row in results:
          ...
          for col_num in xrange(len(row)):
              if col_num == 5 or col_num == 9 or col_num == 8 or col_num == 12 or col_num == 15 or col_num == 17 or col_num == 19:
                ...
                ws.write(row_num, col_num, IdforUser(row[col_num]),font_style4)
              elif col_num == 6 or col_num == 10 or col_num == 13 or col_num == 16:
                ws.write(row_num, col_num, Participate(row[0],row[1],col_num),font_style4)
              else:
                ws.write(row_num, col_num, row[col_num],font_style4)
    except:
       print "Error: unable to fetch stakeholders decisions data"'''
    ...
    columns = [
          (u"decision_id", 3000),
          (u"so_id", 3000),
          (u"solution_option", 8000),
          (u"so_source", 8000),
          (u"so_details", 8000),
          (u"cost_metric", 8000),
          (u"num_participants_served", 8000),
          (u"personnel_cost", 8000),
          (u"facilities_cost", 8000),
          (u"materials_cost", 8000),
          (u"training_cost", 8000),
          (u"other_cost", 8000),
          (u"total_avg_marg_tot_costs", 8000),
          (u"overall_utility_value", 8000),
          (u"cost_per_unit_utility", 8000),
          (u"selected",3000),
          (u"file_name_1", 3000),
          (u"file_attachment_1", 5000),
          (u"file_name_2", 3000),
          (u"file_attachment_2", 5000),  
          (u"file_name_3", 3000),
          (u"file_attachment_3", 5000),  
          (u"file_name_4", 3000),
          (u"file_attachment_4", 5000),  
          (u"link_name_1", 3000),
          (u"link_1", 5000),
          (u"link_name_2", 3000),
          (u"link_2", 5000),  
          ...
          (u"link_4", 5000),    
          (u"so_archived", 3000),
          (u"so_archived_on", 3000),
          (u"so_archived_by", 3000),
          (u"so_unarchived", 3000),
          (u"so_unarchived_on", 3000),
          (u"so_unarchived_by", 3000),
          (u"so_deleted", 3000),
          (u"so_created_on", 3000),
          (u"so_created_by", 3000),
          (u"so_updated_on", 3000),
          (u"so_updated_by", 3000),
    ]
    try:
       cursor2.execute(sql2,{'demo_dec_list' :demo_dec_list})
       results = cursor2.fetchall()
       ...
       for row in results:
          ...
          for col_num in xrange(len(row)):
              if col_num == 15:
                ...
                ws.write(row_num, col_num, row[col_num],date_style4)
              elif col_num == 34 or col_num == 37 or col_num == 40 or col_num == 42:
                ws.write(row_num, col_num, IdforUser(row[col_num]),font_style4)
              else:
                ws.write(row_num, col_num, row[col_num],font_style4)                                                                                    
    except:
    ...
    columns = [
          ...
          (u"sc_id", 3000),
          (u"criterion", 8000),
          (u"sc_textelaboration", 8000),
          (u"user_write_in", 4000),
          (u"sc_created on", 4000),
          (u"sc_created_by", 4000),
          (u"sc_updated_on", 4000),
          (u"sc_updated_by", 4000),
    ]
    try:
       cursor2.execute(sql2,{'demo_dec_list' :demo_dec_list})
       results = cursor2.fetchall()
    ...
    columns = [
          ...
          (u"sc_id", 3000),
          (u"criterion", 8000),
          (u"evaluate_option_against_criterion", 6000),
          (u"sct_created_on", 4000),
          (u"sct_created_by", 4000),
          (u"sct_updated_on", 4500),
          (u"sct_updated_by", 4000),
    ]
    ...
    for m in mapp:
        row_num += 1
        ws.write(row_num, 0, m.dec_id,font_style4)
        ws.write(row_num, 1, m.sol_id,font_style4) 
        ws.write(row_num, 2, m.sol_option,font_style4)
        ws.write(row_num, 3, m.sc_id,font_style4) 
        ws.write(row_num, 4, m.criterion,font_style4)
        ws.write(row_num, 5, m.result,font_style4) 
        cdate = str(m.created_date)
        sliced_cdate = cdate[0:10]   
        ws.write(row_num, 6, sliced_cdate,date_style4)
        ws.write(row_num, 7, IdforUser(m.created_by),font_style4) 
        udate = str(m.updated_date)
        sliced_udate = udate[0:10]
        ws.write(row_num, 8, sliced_udate,date_style4)
        ws.write(row_num, 9, IdforUser(m.updated_by),font_style4) 

    # Evaluation Criteria                                                                                                        i
    ws = wb.add_sheet("Evaluation Criteria")
    row_num = 0 
    cursor2 = database.cursor ()
    sql2 = """SELECT e.dec_id, e.id, opt_id,  fieldname, or_criterion,  e.criterion,  criterion2,  combined, suggested_evam,  data,  weight,  adjusted_weight, lowest_value, highest_value, higher_better, option_value, utility_value,  e.created_date,  e.created_by,  e.updated_date,  e.updated_by,  e.deleted FROM utility_tool_evaluation_criteria e LEFT JOIN utility_tool_evaluation_measures em on e.id = em.eva_id where e.dec_id not in %(demo_dec_list)s and (e.deleted = 'N' or e.deleted is null) order by e.dec_id, e.id"""

    columns = [
          (u"decision_id", 3000),   
          (u"ec_id", 3000),
          (u"so_id", 3000),
          (u"user_added_criterion", 5000),
          (u"overarching_criterion", 15000),
          (u"granular_criterion", 15000),
          (u"ec_textelaboration", 9000),        
          (u"combined_criterion", 15000),
          (u"data_recommended_measure", 15000),
          (u"ec_data_to_collect", 15000),
          (u"imp_scr_adjusted_by_votes", 6500),
          (u"adjusted_weight", 4000),
          (u"likely_lowest_score", 6500),
          (u"likely_highest_score", 6500), 
          (u"higher_better", 6500), 
          (u"so_rating", 6500), 
          (u"criterion_utility_value", 6500),
          (u"ec_created_on", 4000),
          (u"ec_created_by", 4000),
          (u"ec_updated_on", 4000),
          (u"ec_updated_by", 4000),
          (u"ec_deleted", 3000),
    ]
    try:
       cursor2.execute(sql2,{'demo_dec_list' :demo_dec_list})
    ...
    row_num = 0 
    cursor2 = database.cursor ()
    sql2 = """SELECT dec_id, id,  created_by, eva_id,  criterion,  score,  deleted, created_date,  updated_date,  updated_by,  email FROM utility_tool_importance_scores where dec_id not in %(demo_dec_list)s and (deleted = 'N' or deleted is null)"""
    ...
    columns = [
          ...
          (u"ec_id", 3000),
          (u"criterion", 15000),
          (u"importance_score", 4500),
          (u"imp_scr_deleted", 4500),
          (u"imp_scr_created_on", 5000),
          (u"imp_scr_updated_on", 5000),
          (u"imp_scr_updated_by", 5000),
          (u"Email", 0),
    ]
    try:
       cursor2.execute(sql2,{'demo_dec_list' :demo_dec_list})
    ...
    columns = [
          ...
          (u"solution_option", 8000),
          (u"ec_id", 3000),
          (u"common_measure", 10000), 
          (u"data_to_collect", 10000), 
          (u"criterion", 10000),
          (u"evaluate_option_against_criterion", 10000),
          (u"id_created_on", 4000),
          (u"id_created_by", 4000),
    ]
    '''try:
       cursor2.execute(sql2,{'demo_dec_list' :demo_dec_list})
       results = cursor2.fetchall()
    ...
    for m in idn:
        ...
        ws.write(row_num, 1, m.sol_id,font_style4) 
        ws.write(row_num, 2, m.sol_option,font_style4)
        ws.write(row_num, 3, m.ec_id,font_style4) 
        ws.write(row_num, 4, m.measure,font_style4)
        ws.write(row_num, 5, m.data,font_style4)
        ws.write(row_num, 6, m.criterion,font_style4)
        ws.write(row_num, 7, m.result,font_style4) 
        cdate = str(m.created_date)
        sliced_cdate = cdate[0:10]
        ws.write(row_num, 8, sliced_cdate,date_style4)
        ws.write(row_num, 9, IdforUser(m.created_by),font_style4) 
           ...
           for temp in Temp_Mapping.objects.all():
             if scr_id <>  0 and scr_id is not None and scr_id <> ' ' and scr_id <> '':  
                try:
                   ...
                   m = Mapping_Data(dec_id = dec_id, sol_id = temp.sol_id, sol_position = temp.sol_position, sol_option = temp.sol_option, sc_id = scr_id, criterion = criteria, created_date = mapptable.created_date,created_by = mapptable.created_by,updated_date = mapptable.updated_date,updated_by = mapptable.updated_by, archived = temp.archived)   
                   m.save()
