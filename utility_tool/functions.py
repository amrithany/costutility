...
def check_required(request, dec_id):
    ids = {}
    ids2 = {}
    ids3 = {}
    ids4 = {}
    ...
    retval = ''
    try:
       solopt = Solution_Options.objects.get(dec_id=dec_id,archived='N', deleted='N')
    ...
    try: 
       Importance_Scores.objects.get(dec_id = dec_id)
    except ObjectDoesNotExist:
       retval = retval + ',iw'
    except MultipleObjectsReturned:                                                                                                                
       print 'iw multiple objects returned' 
       i = Importance_Scores.objects.filter(dec_id=dec_id).exclude(deleted = 'Y')
...
def further_cal(dec_id, loggedinuser, request):
    ...
    if (onerec.lowest_value == 0 and  onerec.highest_value == 0) or (onerec.higher_better <> 'Y' and onerec.higher_better <> 'y' and onerec.higher_better <> 'N' and  onerec.higher_better <> 'n') or onerec.option_value is None:              
    ...
    for q in qset:
       ...
       if (q.lowest_value == 0 and q.highest_value == 0) or (q.higher_better <> 'Y' and q.higher_better <> 'y' and q.higher_better <> 'N'and  q.higher_better <> 'n') or q.option_value is None:         
       ...
       if solopt == q.sol_option:
          ...
          if req_weight.adjusted_weight  is not None:
...
def IdforUser(user):
    try: 
       u = Users.objects.filter(user = user).latest('startDate')
       randomId = str(u.uniqueRandomId)
    except ObjectDoesNotExist:
       print user 
...
def Participate(dec_id, userid,col_num):
    returnVal = ' '   
    try: 
       u = Stakeholders_Decisions.objects.get(id = userid)
       email = u.email
       u = Users.objects.get(email = email)
       user = u.user
       if col_num == 16: 
          try:
             solopt_count = Solution_Options.objects.filter(dec_id = dec_id, created_by = user).count()
             if solopt_count > 0:
                returnVal = 'Y' 
             else:
                returnVal = ' '  
          ...
          try:
             scr_count = Screening_Criteria.objects.filter(dec_id = dec_id, created_by = user).count()
             if scr_count > 0:
                returnVal = 'Y' 
             else:
                returnVal = ' '  
          ...
          try:
             eva_count = Evaluation_Criteria.objects.filter(dec_id = dec_id, created_by = user).count()
             if eva_count > 0:
                returnVal = 'Y' 
             else:
                returnVal = ' '                                                                                                           
          ...
          try:
             iw_count = Importance_Scores.objects.filter(dec_id = dec_id, created_by = user).count()
             if iw_count > 0:
                returnVal = 'Y' 
             else:
                returnVal = ' '  
...
def TotalST(dec_id,incl_PA):
    returnVal = ' '  
    try: 
       returnVal = Stakeholders_Decisions.objects.filter(dec_id= dec_id).count()
    except ObjectDoesNotExist:
       print 'Id does not exist'  
    if incl_PA == 'Y':
       return returnVal 
...
def LocVal(dec_id):
    returnVal = "http://amritha.pythonanywhere.com/utility_tool/decisions/" + str(dec_id) + "/solution_options/view_solopt_det.html"  
    return returnVal

def StValue(email, created_by, what_to_return, from_where):
    returnVal = ''
    try: 
       s = Stakeholders.objects.get(email = email, created_by = created_by)
       if what_to_return == 0:
          if from_where == 'here': 
             returnVal = s.firstName
          else:
             returnVal = s.id 
       elif what_to_return == 1:
          if from_where == 'here': 
             returnVal = s.lastName
          else:
             returnVal = ''   
       elif what_to_return == 2:
          if from_where == 'here': 
             returnVal = s.title 
          else:
             returnVal = ''  
       elif what_to_return == 3:
          if from_where == 'here': 
             returnVal = s.organisation
          else:
             returnVal = ''  
    except ObjectDoesNotExist:
       print 'prob with stakeholders record'  
       returnVal = ' '   
    return returnVal
...
def getUserfromEmail(email):
    try:
       ...
       print 'prob with user record' 
       user = 'not found'
    return user

def PartValue(dec_id, email, created_by, row_num, col_num):
    ...
    if row_num == 'Y':
       if col_num == 4: 
          ...
          try:
             ...
             print 'prob with iw record'  
    else:
       returnVal = 'X'
       
    return returnVal

def selectOption(dec_id, sol_option):
    chosen = ""
    print "in functions" 
    try:  
