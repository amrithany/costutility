...
def summary_report(request):
    ...
    try:
       stdec_count = Stakeholders_Decisions.objects.filter(dec_id=dec_id).count()
    except:
       stdec_count = 0
    ...
    try:
       cost_utility = Cost_Utility.objects.filter(dec_id = dec_id)
    ...
    styles.add(ParagraphStyle( name="ParagraphTitle", fontSize=12, alignment=TA_CENTER, fontName="Helvetica", textColor= blue,backColor=lightblue))
    styles.add(ParagraphStyle( name="MainParagraph", fontSize=11, alignment=TA_JUSTIFY, fontName="Helvetica"))
    ...
    actualpara = name_decisionmaker + " made a decision about " + title + " using the cost-utility decision-making framework. " + str(stdec_count) + " stakeholders contributed to the decision-making process by suggesting solution options, providing screening criteria, developing a list of evaluation criteria or contributing to the importance scores.  Among " +  str(solopt_count) + " solution options that were evaluated," + so1 + " has the highest overall utility value, indicating that it is the best one to meet the stakeholders’ needs; " + so2 + " has the lowest " + type_of_cost + " cost; and " + so3 + " has the lowest cost-utility ratio, indicating that it provides the highest return on investment." + name_decisionmaker + " chose " + " based on the following rationale or consideration: " + reason + ". For details, please refer to the following pages. " 
...
def about_maker(request):                                                                                                                                  
    ...
    return render(request, 'resources/about_maker.html')
...
def menu(request, dec_id):
    ...
    #print created_by
    #print loggedinuser
    return render(request,'decisions/menu.html',{'dec_title':request.session['dec_title'], 'dec_id':dec_id, 'loggedinuser':loggedinuser, 'created_by' :created_by, 'solopt_allowed':solopt_allowed, 'scrcr_allowed':scrcr_allowed, 'evacr_allowed': evacr_allowed, 'iw_allowed':iw_allowed, 'group_yn':group_yn, 'votes_yn':votes_yn, 'solopt_created':solopt_created, 'scr_created': scr_created, 'eva_created':eva_created, 'evam_created':evam_created, 'iw_created':iw_created, 'decmade_created':decmade_created, 'cu_created':cu_created, 'cost_created':cost_created, 'stsolopt_created':stsolopt_created, 'stscr_created':stscr_created, 'steva_created':steva_created, 'stiw_created':stiw_created, 'setup_created':setup_created, 'stakeholdersNow':stakeholdersNow, 'mapp_created':mapp_created})
...
def add_solopt(request):
    ...
    MFormSet = modelformset_factory(Solution_Options, form=SolOptForm, extra=10)
    if request.method == 'POST':
        ...
        if soloptform.is_valid():
           ...
           if 'submit2' in request.POST:    
              ...
              return HttpResponseRedirect('/utility_tool/decisions/solution_options/add_solopt.html') 
        else:
            print soloptform.errors

    else:
        qset = Solution_Options.objects.filter(dec_id=dec_id)
    ...
    return render(request,'decisions/solution_options/add_solopt.html',{'soloptform':soloptform,'dec_id':dec_id, 'dec_title':dec_title})
'''
...
def add_scr_criteria(request):
    ...
    if request.method == 'POST':
       ...
       while len(a):                                                                                                                                                                                             
          ...
          if temp_list[0] == 'null':
              if temp_list[1] == 'Serves target population (grade level':
                 temp_list[1] = 'Serves target population (grade level, ESL etc.)' 
              m = Screening_Criteria(criterion = temp_list[1], dec_id = request.session['dec_id'], created_by = request.session['user'],created_date = datetime.datetime.now())
              m.save()  
              something_saved = 'yes' 
          elif temp_list[1] == "":
              request.session['map_list'].append(temp_list[0])
              Screening_Criteria.objects.get(id = temp_list[0]).delete()
              something_saved = 'yes'
          else:
             try:
                m = Screening_Criteria.objects.get(id = temp_list[0]) 
    ...
    qset = Screening_Criteria.objects.filter(dec_id=dec_id)
    return render(request,'decisions/solution_options/add_scr_criteria.html',{'qset':qset,'dec_id':dec_id, 'dec_title':dec_title, 'mapping_allowed': mapping_allowed, 'solopt_count':solopt_count,'created_by':created_by, 'loggedinuser':loggedinuser, 'stakeholdersNow':stakeholdersNow})      
...
def add_scr_criteria(request):
    ...
    dec = Decisions.objects.get(pk=dec_id) 
    something_saved = 'no'
    MFormSet = modelformset_factory(Screening_Criteria, form=ScrCriteriaForm, extra=10)
    if request.method == 'POST':
        scrcrform = MFormSet(request.POST,request.FILES,prefix="scrcrform" )
        if scrcrform.is_valid():
           id = scrcrform.save(commit=False)
           for recs in id:
               if recs.criterion == '':
                  recs.delete()
               else: 
           ...
           if 'submit' in request.POST:    
            ...
            print scrcrform.errors

    else:
        qset = Screening_Criteria.objects.filter(dec_id=dec_id)
        scrcrform = MFormSet(queryset = qset,prefix="scrcrform")
        for form in scrcrform:
           form.fields['created_by'].widget.attrs['readonly'] = True
           form.fields['updated_by'].widget.attrs['readonly'] = True
    return render(request,'decisions/solution_options/add_scr_criteria.html',{'scrcrform':scrcrform,'dec_id':dec_id, 'dec_title':dec_title, 'mapping_allowed': mapping_allowed, 'solopt_count':solopt_count,'created_by':created_by, 'loggedinuser':loggedinuser}) 
'''
...
def add_scores(request):
    ...
    if request.method == 'POST':
        ...
        for form in scoresform:
           ...
           #form.fields['created_by'].widget.attrs['readonly'] = True
    return render(request,'decisions/solution_options/add_scores.html',{'scoresform':scoresform,'dec_id':dec_id, 'dec_title':dec_title, 'created_by':created_by, 'loggedinuser':loggedinuser, 'group_yn' :group_yn, 'votes_yn':votes_yn}) 
...
'''
def add_scr_det(request):
    context = RequestContext(request)
    if 'dec_id' in request.session:
       dec_id = request.session['dec_id']
    else:
       dec_id = 0
    MFormSet = modelformset_factory(Screening_Criteria, form=ScrCriteriaForm2, extra=6)
    if request.method == 'POST':
        scrcrform = MFormSet(request.POST,request.FILES,prefix="scrcrform" )
        ...
        qset = Screening_Criteria.objects.filter(dec_id=dec_id)
        scrcrform = MFormSet(queryset = qset,prefix="scrcrform")
    return render(request,'decisions/solution_options/add_scr_det.html',{'scrcrform':scrcrform,'dec_id':dec_id}) 
'''
...
def add_measures(request):
    ...
    if request.method == 'POST':
       ...
       while len(y): 
          ...
          print len(temp_list)
          if len(temp_list) > 7:
             # looping through the solution options and evaluation measures
...
def cost_table(request):                                                                                                                           
    ...
    costs = Cost_Utility.objects.filter(dec_id = dec_id)
...
def cost_setup(request):
    ...
    cost_table = Cost_Utility.objects.filter(dec_id = dec_id)
...
def decision_made(request):                                                                                                                                                                                         
    ...
    if loggedinuser == created_by:
       if group_yn == 'N':
          ...
          group_cal(dec_id, loggedinuser, votes_yn, request)
       further_cal(dec_id, loggedinuser, request)   
...
def register(request):
   ...
   return render_to_response('users/register.html',{'registerform': registerform}, context)
