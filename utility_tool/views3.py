...
from utility_tool.forms import DecisionForm, SolOptForm, SolOptForm2, ScrCriteriaForm, EvaCriteriaForm, LoginForm, RegisterForm, StakeholdersForm, SolOptView, VotesForm, ScoresForm, SolOptArchive, SetupForm, CostSetupForm, DecisionMadeForm, Solopt_Storage
from django.template import loader
from .models import Decisions, Solution_Options, Screening_Criteria, Evaluation_Criteria, Importance_Scores, Users, Stakeholders, Stakeholders_Decisions, MappingTable, SummaryTable, Evaluation_Measures, PA_Setup, EvaluationTable,  Cost_Setup, Cost_Utility, Decision_Made, Detailed_Costs, CBCSE_Screening_Criteria, Master_Screening_Criteria, CBCSE_Evaluation_Criteria, Master_Evaluation_Criteria, Scores_Setup, IdentifyTable, Solution_Options_Storage
import datetime
import json
import xlrd
import xlwt
...
from utility_tool.functions import check_required, group_cal, individual_cal, further_cal, update_text_criteria, redistribution_func
...
def summary_report(request):
    ...
    try:
       ...
       for ss in std:
           try:
              ...
              try:
                 solopt_user = Solution_Options.objects.filter(dec_id=dec_id, archived='N', created_by = user_solopt.user)
                 print 'I AM HERE'
                 solopt_user_count = solopt_user.exclude(created_by = loggedinuser).count()   
    ...
    try:
       ...
       solopt_archive = solopt1.exclude(archived='N')
       sa_count = solopt_archive.count()
    except:
    ...
    if stdec_count > 0 and text <> "":
       ...
       p.add_run("invited to contribute to the following stages of the decision-making process: " + text + "." )
    run = p.add_run()
    run.add_break() 
    run.add_break() 
    p.add_run('Among ' +  p4.number_to_words(solopt_count) + ' solution options that were evaluated, it appears that, before considering costs, ' + so1 + ' yield(s) the highest stakeholder satisfaction, i.e., it (they) best meet(s) your stakeholders’ criteria, earning an overall utility of ' + str(wu) + ' out of 10.')
    run = p.add_run()
    run.add_break()
    run.add_break()
    p.add_run(so2 + ' is (are) the least costly:  ' + type_of_cost + ' is ' + str(co))
    run = p.add_run()
    run.add_break()
    run.add_break()
    p.add_run(so3 + ' provides (provide) the best return on investment because it (they) costs (cost) the least per unit of utility or stakeholder satisfaction:' + str(cur) + '.')
    run = p.add_run()
    run.add_break()
    run.add_break()
    p.add_run(name_decisionmaker + ' chose ' + chosen + ' based on the following rationale or consideration: ' + reason + '. For details, please refer to the following pages.')
    ...
    #print 'st_solopt_contrib_count at bottom'
    #print st_solopt_contrib_count
    #print p4.number_to_words(st_solopt_contrib_count)
    # Solution Options
    ...
    for s in solopt:
       ...
       row_cells[2].text = s.option_details
       counter = counter + 1
    d = document.add_paragraph('')
    run = d.add_run()     
    run.add_break() 
    ...
    if sa_count > 0:
       d = document.add_paragraph('')
       d.add_run('The following additional Solution Option(s) was (were) suggested but was (were) not moved forward in the decision process: ')
       for s in solopt_archive:
...
def send_email(request, st_id):
    ...
    st = Stakeholders.objects.get(pk=st_id)
    strFrom = 'amrithany@gmail.com'
    strTo = st.id
    email = EmailMessage('Subject', 'Body', to=['amritha_mm@yahoo.com'])
    email.send()
    return HttpResponseRedirect('/utility_tool/stakeholders/stakeholders.html')
...
def edit_decision(request, dec_id):
    ...
    if request.method == 'POST':
        ...
        if decform.is_valid():
            ...
            print id.by_when   
            id.save(update_fields=['short_title','title','name_decisionmaker', 'evidence', 'real_dec_yn', 'decision_prob','goal','target_audience','stakeholders', 'participating_stakeholders', 'by_when', 'updated_date','updated_by']) 
...
def dupl_decision(request, dec_id):
    ...
    dec.created_at = datetime.datetime.now()
    dec.pk = None 
    dec.save()
       ...
       for s in Decision_Made.objects.filter(dec_id=dec_id):
          ...
          while len(b):
             for l in b.split(','): 
                ...
                print 'd'
                print d
                oldrec = Solution_Options.objects.get(id = c, dec_id = dec_id)
...
def pa_setup(request):
    ...
    return render(request,'decisions/pa_setup.html',{'dec_id':dec_id,'dec_title':dec_title, 'loggedinuser':loggedinuser, 'stakeholdersNow':stakeholdersNow, 'votesform':votesform,'total_votes':total_votes, 'total_voters':qset_count})
...
def menu(request, dec_id):
    ...
    try:
       solopt = Solution_Options.objects.get(dec_id=dec_id, created_by = loggedinuser, archived='N')
       solopt_created = 'Y'   
    except ObjectDoesNotExist:
       solopt_created = 'N'
    except MultipleObjectsReturned:                                                                                                           
    ...
    try: 
       soloptP = Solution_Options.objects.get(dec_id=dec_id, archived='N')
       soloptP_created = 'Y'   
    except ObjectDoesNotExist:
       soloptP_created = 'N'
    except MultipleObjectsReturned:                                                                                                                
...
def add_solopt_det(request):
           ...
           for recs in id:
              if recs.sol_option <> '':
                 ...
                 try:
                    ...
                    if st2_count > 0:
                       request.session['error_message'] = '"' + recs.sol_option + '" has already been suggested as a Solution Option. If you cannot see it in the list below, the Project Administrator has temporarily archived it. Duplicates have been removed.' 
                       ...
                       #return render(request,'decisions/solution_options/add_solopt_det.html',{'soloptform':soloptform,'dec_id':dec_id, 'dec_title':dec_title, 'mapping_allowed': mapping_allowed, 'scr_count':scr_count, 'err':request.session['error_message'] })
                 except ObjectDoesNotExist:
    ...
    if 'error_message' in request.session: 
       return render(request,'decisions/solution_options/add_solopt_det.html',{'soloptform':soloptform,'dec_id':dec_id, 'dec_title':dec_title, 'mapping_allowed': mapping_allowed, 'scr_count':scr_count,'created_by':created_by, 'loggedinuser':loggedinuser, 'err':request.session['error_message'], 'sol_permC':sol_permC})
    else:
       return render(request,'decisions/solution_options/add_solopt_det.html',{'soloptform':soloptform,'dec_id':dec_id, 'dec_title':dec_title, 'mapping_allowed': mapping_allowed, 'scr_count':scr_count,'created_by':created_by, 'loggedinuser':loggedinuser, 'sol_permC':sol_permC}) 
...
def solopt_storage(request):
    ...
    return render(request,'decisions/solution_options/solopt_storage.html',{'dec_title':dec_title, 'sform':sform})
...
def assign_tasks(request):                                                                                                                                                                                         
    ...
    return render(request,'decisions/solution_options/assign_tasks.html',{'st_dec': stakeholders_decisions, 'stdec_type':stdec_type, 'dec_id':dec_id, 'dec_title':dec_title, 'loggedinuser':loggedinuser, 'user_email':user_email, 'impexists':impexists})
...
def add_scores(request):
    ...
    if request.method == 'POST':
        ...
        for form in scoresform:
           ...
           #form.fields['created_by'].widget.attrs['readonly'] = True
    return render(request,'decisions/solution_options/add_scores.html',{'scoresform':scoresform,'dec_id':dec_id, 'dec_title':dec_title, 'created_by':created_by, 'loggedinuser':loggedinuser, 'stakeholdersNow':stakeholdersNow, 'thinking':thinking}) 
...
def handsontable(request):
    ...
    return render(request,'decisions/solution_options/handsontable.html', {'dec_id':dec_id, 'dec_title': dec_title, 'mapping':maptable, 'solopt':solopt, 'scrcr':scrcr, 'scrcr_count':scrcr_count, 'map_list':map_list, 'loggedinuser':loggedinuser})
...
def summary(request):
    ...
    try:
       ...
       return render(request,'decisions/message.html', {'dec_id':dec_id, 'loggedinuser':loggedinuser, 'mess':'You have not entered the setup information for scores so you cannot view this screen.'})
    ...
    if group_yn == 'N':  
       ...
       for q in scores:
          ...
          if votes_yn == 'Y': 
             if  q.votes is None:                                                                                                                                                                             
                return render(request,'decisions/message.html', {'dec_id':dec_id, 'loggedinuser':loggedinuser, 'mess':'You have not assigned the number of votes allocated to each stakeholder.'})
...
def utility_results(request):
    ...
    qset1 = Evaluation_Measures.objects.filter(dec_id=dec_id).order_by('opt_id')
...
def idn_measures(request):
    ...
    something_saved = 'no'
    try: 
       evatable = IdentifyTable.objects.get(dec_id=dec_id)
       table =  evatable.table
    except ObjectDoesNotExist:
       table = 'doesnotexist'
          ...
          for e in evacr:
             try:
                 ...
                 if e.criterion2 is not None and e.criterion2 <> '':
                    ...
                    criterion = e.criterion 
                 m = Evaluation_Measures(opt_id = s.id, sol_option = s.sol_option, archived = s.archived, deleted = e.deleted, eva_id = e.id, criterion = criterion,  dec_id = dec_id,created_by = request.session['user'],created_date = datetime.datetime.now())
                 something_saved = 'yes' 
                 m.save() 
...
def cost_table(request):                                                                                                                           
    ...
    costs = costs1.exclude(archived = 'Y')
...
def decision_made(request):                                                                                                                                                                                         
    ...
    context = RequestContext(request)
       ...
       # using a function here
    retval = check_required(request, dec_id, loggedinuser, created_by)
    sol_mesg = ''
    eva_mesg = ''
    iw_mesg = ''
    evm_mesg = ''
    ...
    if loggedinuser == created_by:
       ...
       retval = further_cal(dec_id, loggedinuser, request)   
       if retval == 'em':
          evm_mesg = '- Specify the evaluation measures and add the average rating/score for each Solution Option in the "Evidence-gathering to Evaluate Options" step'
    ...
    if request.method == 'POST':
       ...
       dec_made.save(update_fields=['sol_option','reason', 'none','primary_factor','updated_by','updated_date'])
       dec.updated_by = request.session['user']
       dec.updated_date = datetime.datetime.now()
       dec.save(update_fields=['updated_by','updated_date'])            
       if 'submit' in request.POST:                                                                                                            
    ...
    return render(request,'decisions/solution_options/decision_made.html',{'query':query,'dec_id':dec_id, 'dec_title':dec_title, 'cost_utility':cost_utility, 'loggedinuser':loggedinuser, 'created_by':created_by, 'cost_text':cost_text, 'reason': reason, 'primary_factor' : primary_factor, 'sol_opt': sol_opt, 'noneX':none, 'wu':wu,'so1':so1, 'co':co, 'so2':so2, 'cur':cur, 'so3':so3, 'qcost':onerec.cost, 'more_than_one':more_than_one }) 
...
def import_solopt(request, uploaded_file_url):
    if 'dec_id' in request.session:
       dec_id = request.session['dec_id']
    else:
       dec_id = 0
