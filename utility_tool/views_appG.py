...
def export_all(request):     
    ...
    row_num = 1 
    cursor2 = database.cursor ()
    sql2 = """SELECT id,  st_id,  dec_id,  iw_type,  iw_date,  evacr_type,  evacr_date,  scrcr_type,  scrcr_date,  solopt_type,  solopt_date,  created_date,  created_by,  updated_date,  updated_by,  votes,  deleted FROM utility_tool_stakeholders_decisions where dec_id not in %(demo_dec_list)s"""      
    ...
    row_num = 1 
    cursor2 = database.cursor ()
    sql2 = """SELECT id, criterion,criterion2,dec_id,fieldname,created_date,created_by, updated_date, updated_by FROM utility_tool_screening_criteria where dec_id not in %(demo_dec_list)s"""                    
    columns = [
          (u"Id", 3000),
          (u"Criterion", 8000),
          (u"Criterion2", 8000),
    ...
    row_num = 1 
    cursor2 = database.cursor ()
    sql2 = """SELECT * FROM utility_tool_evaluation_criteria where dec_id not in %(demo_dec_list)s"""
    ...
    try:
       ...
       for row in results:
          row_num += 1
          for col_num in xrange(len(row)):
              if col_num == 13 or col_num == 15:
                ws.write(row_num, col_num, row[col_num],date_style4)
              elif col_num == 14 or col_num == 16:
                ws.write(row_num, col_num, IdforUser(row[col_num]),font_style4)
              else:
                ws.write(row_num, col_num, row[col_num],font_style4)
    except:                                                                                                                                             
    ...
    row_num = 1 
    cursor2 = database.cursor ()
    sql2 = """SELECT * FROM utility_tool_importance_scores where dec_id not in %(demo_dec_list)s"""
